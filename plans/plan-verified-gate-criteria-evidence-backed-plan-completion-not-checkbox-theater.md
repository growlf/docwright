...
| 2026-07-14 | Step 2 done — wired the resolver into checkCompletionGate, backward-compatible. completion-gate.ts now imports gate-criteria (parseFrontmatter added); a new planGateEvidence(text) builds evidence purely from the plan's own frontmatter (frontmatter fields + hasPendingSteps + gate_evidence/gate_attestations maps — keeps the gate a pure text fn; file_exists/cmd rely on recorded evidence from step 3). The old 'count unchecked [ ]' block is replaced by hasGateSection() + auditGateCriteria(): unbound criteria keep EXACT legacy semantics (must be [x]) and pure-legacy plans still get the identical historical message, so the gate-parity contract is unchanged; bound criteria are derived (tier1, typed box ignored) or attested-and-not-contradicted (tiers 2/3), with a per-criterion 'why unmet' message when any binding is involved. isReadyForHumanCompletion() also switched to the resolver: ready when all tier-1 (machine) criteria pass, treating human/unbound criteria as the human's remaining Certify/Attest action (matches its docstring intent). 12 new dispatch tests (backward-compat incl. exact legacy message + no-gate-section; tier-1 derive incl. checked-but-false still blocks; human attestation; tier-2 contradiction; mixed per-criterion; isReady machine-vs-human). Verified: tsc clean; dispatch 532 passing (was 520); integration 28 passing INCLUDING gate-parity (MCP==UI refusal); mcp all green. Next: step 3 (evidence recorder — the agent verify-and-record path, cmd allowlist). | NetYeti |
| 2026-07-14 | Step 3 done — evidence recorder (agent verify-and-record path). New verify_gate_criteria MCP tool (src/mcp/tools/gate_verify.ts, registered in mutation_index.ts) + Web UI POST /api/lifecycle/verify-gate endpoint. Both share a pure dispatch core to avoid drift: gate-criteria.ts gained evaluateMachineCheckLive() (runs a machine check via injected fs/cmd effects), machineCheckOf() (the check under a tier-1 binding or a tier-2 human+check sub-check), buildGateEvidence() (iterates criteria, records machine ones keyed by id with commit+ts, skips human/unbound, warns on missing id, merges onto existing), and GATE_CMD_ALLOWLIST (fixed name→npm-script map: typecheck/lint/roadmap-check/test-dispatch). Security: cmd:<name> only runs allowlisted names — the default runner returns null (records satisfied:false) for anything else, never spawning. Nested gate_evidence map is written surgically by new dispatch setFrontmatterMap() (preserves other fields + comments; js-yaml block replace), re-exported via mcp/lib/frontmatter. Closing loop verified: recorded evidence lets the pure checkCompletionGate satisfy cmd:/file_exists: criteria it can't run itself. Tests: 9 dispatch (setFrontmatterMap round-trip; machineCheckOf; evaluateMachineCheckLive; buildGateEvidence skip/merge/warn; recorded-evidence→gate) + 6 mcp (records keyed by id, disallowed cmd never executes, missing file false, recorded→gate passes, missing plan, allowlist has no shell metachars); added test/mcp/gate_verify.test.ts to the test:mcp script list. Verified: tsc clean; dispatch 541 passing (was 532); all mcp suites green incl. new recorder; integration 28; webui vite build clean (endpoint compiles). Next: step 4 (human attestation — ACL + audit + cross-check). | NetYeti |
