...
| 7 | Cutover — GH canonical for dev issues | One-time flip: `ISSUES_SOURCE=github` default on. Local `issues/` retained (archived), no longer written. Parallels/instances resume by pulling the tagged release + `main`. | ✅ Done |
| 8 | Retire local issue machinery + clarify docs | Retire `validate_issue_workflow` (pre-commit), dispatch linter issue rules, the local `github_issue:` convention. Clarify CLAUDE.md invariant + PROJECT.md issue-model + AGENTS.md capture directive. | ✅ Done |
...
| 2026-07-13 | Step 7 CUTOVER done + VERIFIED on the dogfood dev instance (dogfood 8bc4c41). Root-caused a deploy drift first: the dev instance (docwright-docwright-1) had been running docker-compose.prod.yml (published :latest image, project-dir mounted as /vault only) instead of the intended dogfood dev setup — so it ran OLD image code and ignored ISSUES_SOURCE. Restored it to docker-compose.yml (dev): .:/app source mount (dogfood code live, no rebuild) + Vite dev server; added GH env pass-through to the dev compose (ISSUES_SOURCE/DOCWRIGHT_GH_* from gitignored .env, default local so no other instance changes). Set ISSUES_SOURCE=github + repo/project/token in .env, rebuilt + swapped the container. VERIFIED reading from the board via a divergence test: set a local file demand_count=55555 while the board Demand stayed 1; /api/status reported 1 and no 55555 anywhere → issues/heatmap/demand come from the GitHub Project, not local files. Reverted the sentinel. Code default left ISSUES_SOURCE=local (explicit per-instance opt-in; the 3 prod instances degrade to local safely). Other instances (csdocs/erp/msp) = deferred cleanup cycle per decision 3. NEXT: Step 8 (retire issues/ pre-commit validate_issue_workflow + dispatch linter issue rules + local github_issue convention; clarify CLAUDE.md invariant, PROJECT.md issue-model, AGENTS.md capture directive). | NetYeti |
| 2026-07-13 | Step 8 done (dogfood 6de671d) — ALL 8 STEPS COMPLETE. Retired the local issue machinery WITHOUT deleting it (git-native vaults keep the model): pre-commit validate_issue_workflow now skips when the repo is GH-canonical (committed issues.migration-map.json marker present) — killing the issues/ commit churn that WAS the cyclic reference; dispatch linter issue-status + issue-milestone rules gated on ISSUES_SOURCE=github. Docs clarified: CLAUDE.md invariant #3 (git = governance-of-record; GitHub Issues+Project = dev tracker for code projects; non-code vaults keep git-native issues), AGENTS.md capture directive (capture → GitHub, no local github_issue backlink to maintain), PROJECT.md issue-model note. Both models coexist, flag/marker-gated. Verified: tsc clean; 484 dispatch tests (2 new gating tests); pre-commit syntax + full hook suite green. PLAN FUNCTIONALLY COMPLETE (8/8 steps + live migration + parity green + dev cutover verified). Remaining for BDFL: final completion attestation (AI cannot set status:completed) + Phase Gate human sign-off; token rotation + fine-grained instance token (BDFL to handle); other instances (csdocs/erp/msp) cutover = deferred cleanup cycle; then one dogfood→main PR. | NetYeti |
| 2026-07-13 | Other-instances cutover RESOLVED (BDFL decision: update-to-new-version, keep git-native). csdocs/erp/msp are content vaults (Projects/csdocs, cs-erp-images, msp-pilot-vault), NOT the code project — per scope they keep the git-native issues/ model. Verified the auto-update path is sound + safe: Watchtower is running and all three carry watchtower.enable=true (auto-pull new :latest); none have ISSUES_SOURCE/DOCWRIGHT_GH env set → after they pull the pivot-aware image they default to local (getIssueSource → 'local', unit-tested), so their issue model is unchanged. Therefore the content-vault cutover is a no-op that happens automatically once #402 merges → main → a new :latest publishes (BDFL release call). No manual per-instance action needed or taken. | NetYeti |
| 2026-07-13 | RELEASED v0.5.3 — the pivot is SHIPPED. Cut a release/v0.5.3 branch off dogfood (bumped VERSION + root package.json + src/webui/package.json 0.5.2→0.5.3; raw dogfood→main #402 was closed — it trips the CI branch-policy typed-prefix gate; release/ branch clears it). PR #403 (release/v0.5.3→main) CI green (branch-policy + tests + docker build; fixed a version-consistency-gate failure by syncing webui package.json), squash-merged to main (repo enforces linear history — no merge/rebase). Tagged v0.5.3 on the merged main commit (9dd49b7) + pushed; release build SUCCESS → ghcr.io/growlf/docwright:v0.5.3 + :latest published. Watchtower will roll :latest to the content vaults (csdocs/erp/msp) automatically, keeping them git-native. FOLLOW-UPS (BDFL): (1) dogfood now diverges from main (main = 1 squash commit; dogfood = 241 originals, same content) — dogfood wants a reconcile/reset to main for future work (shared-branch history rewrite — not done autonomously); (2) final plan completion attestation (status:completed) + Phase Gate sign-off remain human-gated; (3) token rotation + fine-grained instance token at BDFL convenience. | NetYeti |
