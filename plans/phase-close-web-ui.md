...
| --- | --- | --- | --- |
| 1 | Phase readiness indicator on /status | Detect when every plan for the current phase has `status: completed` (reuse the phase/plan data the /status page already loads) and show a "Phase N ready to close — <summary of completed plans>" banner. Verify: with all phase-N plans completed the banner appears; with a pending plan it does not. | ⏳ Pending |
