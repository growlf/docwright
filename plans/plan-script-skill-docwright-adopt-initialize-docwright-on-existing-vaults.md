...
| 7a | Tag existing skills with `distributable:` field | Add `distributable: true` or `distributable: false` to the frontmatter of every `SKILL.md` in `$DOCWRIGHT_PATH/.opencode/skills/`. Skills appropriate for client vaults (e.g., `docwright-git`, `docwright-proposal`, `docwright-lifecycle`, `endsession`) get `true`. DocWright-internal skills (`docwright-project`, `docwright-infra`, `docwright-backup`, `docwright-session-start`, `docwright-security`) get `false`. Default for any untagged skill is `false` (opt-in, not opt-out). Must complete before Step 7b. | ⏳ Pending |
...
- [ ] Step 7a: Every SKILL.md in `.opencode/skills/` has an explicit `distributable:` field; no untagged files remain
- [ ] Step 8: Re-running `adopt --upgrade` after modifying a managed file prompts for resolution; unmodified files update silently
- [ ] Step 8: Run `adopt --upgrade` **twice in a row** with no changes between runs — second run produces zero "user-modified" prompts (verifies manifest entries are refreshed after each overwrite)
- [ ] Step 8: Simulated new-version file (add a file to DW templates with `distributable: true`, run `--upgrade`) — new file appears in vault and manifest; file without `distributable: true` does not appear
...
| 2026-06-17 | Applied 10 code-review findings: VAULT_TARGET guard mandatory in Step 1; manifest hash-update requirement bolded in Step 8; distributable filter + .gemini/settings.json gitignore added to Step 7; Step 7a added to tag existing skills; wipe lists updated in Step 12; author-role added to frontmatter | NetYeti |
