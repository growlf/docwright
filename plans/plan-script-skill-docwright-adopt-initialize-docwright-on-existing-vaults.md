...
|------|--------|---------|--------|
| 1 | Fix js-yaml path in `install-hooks.sh` | Replace `cp "$SOURCE_HOOK" "$HOOK_TARGET"` with a `sed` substitution that rewrites `require('js-yaml')` → `require('$DOCWRIGHT_PATH/node_modules/js-yaml')` in the generated hook. Both quote styles. Verify smoke test still passes. Pre-condition for all hook-install calls downstream. | ⏳ Pending |
