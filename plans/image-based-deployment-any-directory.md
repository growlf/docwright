...
| 2026-07-10 | Phase 3 complete (3.1-3.2 done; 9/14). 3.1: scripts/scaffold-deploy.ts (npm run deploy:scaffold) generates a deploy dir for any vault path (prod compose + filled .env + app.env), editing no tracked repo file; verified by scaffolding into a fresh dir and docker compose up -d booting healthy (200) with the repo staying clean. Caught and fixed a real bug: app.env.example set ORIGIN= (empty), which crashes adapter-node at startup (it rejects empty string) — so a fresh scaffold would have crashed on first up. Fixed: ORIGIN + proxy headers are commented (unset) by default; documented that empty ORIGIN is forbidden and ORIGIN is required once auth is on. 3.2: rewrote docs/docker.md for the image-based flow (scaffold to prod compose to pull-update; two-file config split; ORIGIN/CSRF + reverse-proxy headers; Watchtower); removed the stale Phase-1 vite-dev limitation note; .env.example now flagged dev/launcher-only and points production at the scaffold. Verified: production path has zero git clone/build; build only appears in dev/local-build sections. Next: Phase 4 (live cutover of csdocs/erp/msp to the prod compose) — authorized by BDFL for the cutover demo. | NetYeti |
| 2026-07-11 | Phase 4 cutover done for all three dev-cloud instances (4.1, 4.2 done; 11/14). csdocs (:5274), erp-images (:5275), msp (:5276) each migrated from the throwaway ghcr:v0.4.11 dev-server pin to docker-compose.prod.yml on the docwright:local adapter-node image, with a per-instance app.env (container config copied verbatim from the instance .env so the bcrypt password survives) plus a docker-compose.ssh.yml override mounting the per-repo SSH deploy key. Each verified: healthy, pid1 node build, only /vault + the ssh key mounted, and LOGIN returns 303 (ORIGIN set to http://NAME.bms.local:PORT per DOCWRIGHT_ALLOWED_HOSTS; bcrypt intact, password confirmed). Cross-origin login still 403 (CSRF protection preserved). Discovered HOST_HEADER=host alone does not work (adapter-node defaults protocol to https, mismatching http) so explicit ORIGIN is required. 4.3 partial: removed the throwaway docker-compose.pin-v0.4.11.yml files, disabled auto-release-update.sh, confirmed zero cron references. REMAINING for 4.3: enabling Watchtower and repointing instances to a published ghcr digest is deferred until the Phase 5 release publishes an adapter-node image — the instances are on the local docwright:local tag which is not registry-pollable, so pull/Watchtower cannot be verified until then. ASSUMPTION to confirm with BDFL: instance access URLs are http://NAME.bms.local:PORT (from ALLOWED_HOSTS + direct port, no TLS/proxy on this host); if a different URL/scheme is used, ORIGIN in that instance app.env needs updating or login form POSTs will 403. | NetYeti |
| 2026-07-11 | Phase 5 to the release boundary. 5.1 done: clean-room acceptance (fresh scaffold boots healthy, zero repo edits, docker diff clean) and GH #288 closed + local issue resolved. 5.2 (completion PR) opened as PR #307 to main. Note on divergence: dogfood had diverged from main (main carried the live-ai work as a squash + 6 newer commits incl a human-approved proposal move; dogfood carried the unsquashed history). A dogfood->main merge tripped the proposal-approval gate (correctly — AI may not commit an approval flip), so instead I based the release branch on origin/main and applied ONLY the image-based delta (19 files) as one clean commit bf13f6a — main's approvals stay in the base, no flip. PR #307 is now MERGEABLE (Branch policy passed; Docker build + Lint/Test running). STOPPING at merge/tag per governance: the human squash-merges #307 then tags v0.4.12 (publishes the adapter-node ghcr image). Remaining after that: finalize 4.3 (repoint the 4 instances from docwright:local to the published ghcr digest + enable Watchtower) and close the plan. Also outstanding: dogfood is still behind main and should be reconciled separately (that merge carries main's human-approved proposal, so it needs a HUMAN-APPROVED attestation). | NetYeti |
| 2026-07-11 | Plan functionally complete — all 14 steps done (14/14). BDFL merged PR #307 to main (main at 0.4.12). The v0.4.12 tag had not reached the remote (merge landed but tag push did not), so I pushed v0.4.12 on main HEAD per the stated intent; the tag CI published ghcr.io/growlf/docwright:v0.4.12 and :latest (digest sha256:2cd5b315...). Finalized 4.3: repointed all four instances off the interim docwright:local onto the published image — the three pilots (csdocs/erp-images/msp) to ghcr:latest (auto-update channel), the dogfood dev box to pinned ghcr:v0.4.12 — each verified healthy with login 303. Enabled Watchtower (label-scoped, outbound-only, running, first check ~5 min). Verified the pull mechanism (docker compose pull of :latest, exit 0). Legacy retired earlier (pins removed, cron disabled). REMAINING: plan status -> completed is a human governance action (AI must not self-complete); ready for BDFL to close via the UI Complete button or transition_to_completed with attestation. Also still outstanding (separate task): reconcile dogfood with main (carries a human-approved proposal, needs HUMAN-APPROVED). ORIGIN assumption for pilots (http://NAME.bms.local:PORT) still to be confirmed against the real access URLs. | NetYeti |
