...
> verify_plan_tests (research-smoke fixed in #145/PR #164). All boxes verified.
...
- [x] Human approval gate (`DOCWRIGHT_CONTRIB_APPROVED`) verified non-forgeable by AI
      — BDFL signed off 2026-07-05 (NetYeti; recorded in commit b856ba6 via the Web UI and
      reconfirmed in-session): the env var is human-set server deployment property, and the
      rejection path without it is covered by `contribution.test.ts`
