...
- [x] `npm run test:webui` passes (96 tests — pre-existing suite, unrelated to this plan's changes; run to confirm no regression since no automated suite covers the report modal itself)

### Gate Criteria

- [x] All Step Verification checkboxes are passing
- [x] All Integration & Regression checkboxes are passing
...
- [x] Human test certification (`tests_human_reviewed`) — certified by NetYeti via the Web UI
