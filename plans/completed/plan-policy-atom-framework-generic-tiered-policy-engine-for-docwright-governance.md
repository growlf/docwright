...
| `governance-writes.md` | `governance-writes` | judgment | AI preamble only — no MCP wiring | `plan`, `proposal`, `doc` | ✅ Step 3 | n/a |
| `lifecycle-gate.md` | `lifecycle-gate` | deterministic | `write_plan`, `update_plan_status` via DOCWRIGHT_ATOM_ROUTING | `plan.completing`, `plan.approved` | ✅ Step 3 | ⏳ |
| `no-plaintext-creds.md` | `no-plaintext-creds` | deterministic | hook only | `git-commit`, `plan`, `proposal` | ✅ Step 3 | ⏳ |
| `no-secrets.md` | `no-secrets` | deterministic | hook only | `git-commit` | ✅ Step 3 | ⏳ |
