...
| 3.3 | Migrate chat panel | Behind `LIVE_AI_CHAT`: chat sends via `prompt_async`, renders live from relay — full unfiltered stream (reasoning + tools visible). E2E per 3.2 recipe, **plus failure path: stub `prompt_async` to return 500 → panel shows an explicit error state within 5s (not indefinite "connecting…")**. Verify: e2e green. | ⏳ Pending |
