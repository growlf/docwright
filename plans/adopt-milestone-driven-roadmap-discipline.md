...
| 5 | W0: Fix step-table pipe corruption (bugs #272, #318) | `https://github.com/growlf/docwright/issues/367` — update_step/append_history corrupt rows whose cells contain escaped pipe characters. Same defect class in a second path: plan-review `extractSteps` (src/webui/src/lib/server/plan-review-live.ts, and the legacy /api/plan-review parser) truncates piped cells, feeding reviewers cut-off prompts (https://github.com/growlf/docwright/issues/373, observed on this plan 2026-07-10). Fix with one shared escape-aware row parser. This sits directly under W1's migration path. Done when: regression test with piped content in Action/Details cells passes on update_step, append_history, and buildReviewPrompts | ⏳ Pending |
