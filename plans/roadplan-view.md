...
parent_plan: "plans/separate-dev-tracking-milestones-and-beta-channel.md"
...
|------|--------|---------|--------|-------|--------|
| 1 | Backend API updates | Update `/api/status` to scan the `issues/` directory and return a structured `roadplan` object. | ⏳ Pending | — | feat/roadplan-view |
| 2 | Frontend Web UI updates | Add a `Roadplan` tab to the status page and render current, next, and future milestones. | ⏳ Pending | — | feat/roadplan-view |
| 3 | CLI generator script | Create `scripts/generate-roadplan.ts` to update `docs/roadmap.md` dynamically from the vault state. | ⏳ Pending | — | feat/roadplan-view |
| 4 | Verification & Tests | Implement unit and integration tests for version-sorting and roadplan generation. | ⏳ Pending | — | feat/roadplan-view |
