---
title: "Phase 3 UI Polish Bundle — Panels, Tags, Navigation, Wikilinks, and Deferred Polish"
author: NetYeti
created: 2026-06-06
tags:
  - ui
...
Nine Phase 2 deferred proposals share the same unblocking conditions: a stable
unified panel layout, the Phase 3 dispatch module (vault-wide wikilink index,
Forgejo ACL integration), and the activity-bar settings panel. Tracking them as
nine separate proposals creates noise with no actionable distinction between
them — none can ship before Phase 3 foundations land.
...
- Opens a standalone policy browsing area: categorized listing grouped by domain,
  quick filter by title/tag/category, visual badge showing policy count
...
- Tag-picker component in the properties pane: autocomplete from vault-wide tag set,
  visual chips, lowercase normalization on save
- Vault-wide tag index (`_tags.json`) rebuilt on document changes
- Tag browser in activity bar: alphabetical list with document counts, clickable to
...
- On rename via Web UI or dispatch module: reads `_backlinks.json` to find all
  documents referencing the old path
...
- Lifecycle guardrail: warns and requires confirmation before moving a file to an
  unrecognised directory that would break lifecycle rules
...
- Source priority: Forgejo team membership API → `contributors.json` vault config →
  names seen in existing frontmatter (derived, zero-config fallback)
...
- Acknowledgement state: mark a relationship "reviewed — no action" (stored in
  `.docwright/collation-ack.json`, gitignored), prevents re-surfacing on next save
