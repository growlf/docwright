...
5. **Plan-Scoped Branch Discipline**: Work executing a plan (`plans/*.md`) keeps ALL its changes on a long-lived integration branch (e.g. `dogfood`, which the dogfood container serves) until the plan is finished — one PR per step merged into that branch, **never** step-PRs to `main`. Only when the plan completes does a single integration→`main` PR land it, and that completion PR **must include at minimum a patch-level version bump**. `main` never carries a half-finished plan. Non-plan one-offs (isolated fixes/docs/chores) still use the normal trunk flow. Attach plans to a milestone / sub-milestone for roadmap tracking.
